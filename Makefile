...
invoke-test-logger-local:
	$(VENV_RUN); LAMBDA_NAME=$(shell awslocal cloudformation list-exports --region=$(REGION) --query="Exports[?Name=='test-logger-lambda-name'].Value" --no-paginate --output text); \
		awslocal lambda invoke --function-name $$LAMBDA_NAME --payload '{"message": "hello world"}' --cli-binary-format raw-in-base64-out output.txt

invoke-test-logger-ci:
	$(VENV_RUN); LAMBDA_NAME=$(shell awslocal cloudformation list-exports --region=$(REGION) --query="Exports[?Name=='test-logger-lambda-name'].Value" --no-paginate --output text); \
		awslocal lambda invoke --function-name $$LAMBDA_NAME --payload '{"message": "hello world"}' output.txt
...
deploy-ci: deploy-external-resources-local invoke-test-logger-ci
	$(VENV_RUN); cd deployments/cdk; cdklocal deploy --app "python local_app.py" DataPipelineStack --require-approval never

test-integration: clean install
	$(VENV_RUN); pytest tests/integration -v
